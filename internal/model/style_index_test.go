...
func TestStyleIndexAddAndGet(t *testing.T) {
	...
	idx.Add("minecraft:stone", "", StyledBlock{})
	idx.Add("minecraft:oak_fence", "water=true", StyledBlock{IsFullBlock: false})
	...
	tests := []struct {
		...
		{name: "existing key no props", id: "minecraft:stone", props: "", want: StyledBlock{}, wantOk: true},
		{name: "existing key with props", id: "minecraft:oak_fence", props: "water=true", want: StyledBlock{IsFullBlock: false}, wantOk: true},
...
func TestStyleIndex_Add_ZeroValue(t *testing.T) {
	...
	require.True(t, ok)
	require.Equal(t, StyledBlock{}, v)
}

func TestStyleIndexGetWithColonInProperties(t *testing.T) {
	idx := NewStyleIndex()
	idx.Add("minecraft:stone", "mod:variant", StyledBlock{})

	got, ok := idx.Get("minecraft:stone", "mod:variant")
	require.True(t, ok)
	require.Equal(t, StyledBlock{}, got)
}
